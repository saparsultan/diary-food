...
const RecipeCard = ({ idRecipe }) => {
  ...
  // if(item !== undefined) {
  //   let initvalue = 0;
  //   const sumCalories = item?.products.reduce(
  //     (acc, current) => acc + +current.calories,
  //     initvalue
  //   );
  //   setSumCalories(sumCalories)
  
  //   const sumProteins = item?.products.reduce(
  //     (acc, current) => acc + +current.proteins,
  //     initvalue
  //   );
  //   setSumProteins(sumProteins)
  
  //   const sumFats = item?.products.reduce(
  //     (acc, current) => acc + +current.fats,
  //     initvalue
  //   );
  //   setSumFats(sumFats)
  
  //   const sumCarbs =item?.products.reduce(
  //     (acc, current) => acc + +current.carbs,
  //     initvalue
  //   );
  //   setSumCarbs(sumCarbs)
    ...
    <div className="card__container">
      <div className="card">
        ...
        <div className="card-main">
              ...
              <div className="nutri-value">
                <div className="nutri-value__info">
                  <img
                    ...
                    height="20px"
                  />
                  <div className="nutri-value__sum">{sumCalories && sumCalories}</div>
                </div>
          ...
          <p className="card-p">
            Куриное филе нарезать удобными кусочками и обжарить на растительном
            масле. Лук и сладкий перец очистить и нарезать. Чеснок измельчить. Я
            не большой любитель жарить все на сковороде, готовлю так, как
            привыкла - в глубокой посуде, а вы поступайте по-своему. Добавить в
            сотейник сладкий перец и лук, немного все обжарить до мягкости.
            Блюдо следует периодически помешивать. Посолить по вкусу.
          </p>
        </div>
      </div>
    </div>
