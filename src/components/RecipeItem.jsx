...
const RecipeItem = ({ data }) => {
  let initvalue = 0;
  const sumCalories = data.products.reduce(
    (acc, current) => acc + +current.calories,
    initvalue
  );

  const sumProteins = data.products.reduce(
    (acc, current) => acc + +current.proteins,
    initvalue
  );

  const sumFats = data.products.reduce(
    (acc, current) => acc + +current.fats,
    initvalue
  );

  const sumCarbs = data.products.reduce(
  ...
  console.log("sumWithInitial", sumCalories);

  return (
    <div className="recipe">
      <div className="recipe__title">{data?.name}</div>
      ...
      <div className="nutri-value__img">
        <img src={data?.image} alt={data?.name} width="322px" height="200px" />
      </div>
